...
export const myLoansSlice = createSlice({
	...
	reducers: {
		...
		updateLoan: (state, action) => {
			const { loanID, transactionHash, loanInfo, swapTransactionHashWithLoanID } = action.payload;
			...
			if (loan != null) {
				...
				// TODO: make a new action out of this.
				if (swapTransactionHashWithLoanID && transactionHash && loanInfo.loanID) {
					state.loans[loanInfo.loanID] = state.loans[transactionHash];
